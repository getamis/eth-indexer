...
// UpdateBlocks provides a mock function with given fields: ctx, blocks, receipts, events, reorg
func (_m *Manager) UpdateBlocks(ctx context.Context, blocks []*types.Block, receipts [][]*types.Receipt, events [][]*types.TransferLog, reorg *model.Reorg) error {
	ret := _m.Called(ctx, blocks, receipts, events, reorg)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, []*types.Block, [][]*types.Receipt, [][]*types.TransferLog, *model.Reorg) error); ok {
		r0 = rf(ctx, blocks, receipts, events, reorg)
	} else {
		r0 = ret.Error(0)
	}
