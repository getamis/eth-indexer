...
func (s *subscription) insert(ctx context.Context, events []*model.Transfer) (err error) {
	...
	defer func() {
		for group, addrs := range totalBalances {
			for token, d := range addrs {
				...
				if mr, ok := totalMinerReward[group]; ok && token == model.ETHAddress {
					tb.MinerReward = mr.String()
				}
				if ur, ok := totalUncleReward[group]; ok && token == model.ETHAddress {
	...
	for _, e := range events {
		...
		if hasFrom {
			...
			if f, ok := fees[gethCommon.BytesToHash(e.TxHash)]; bytes.Equal(e.Address, model.ETHBytes) && ok {
				if feeDiff[from] == nil {
		...
		if hasTo && !bytes.Equal(e.From, RewardToMiner.Bytes()) && !bytes.Equal(e.From, RewardToUncle.Bytes()) {
		...
		// Record miner reward
		if to := gethCommon.BytesToAddress(e.To); bytes.Equal(e.From, RewardToMiner.Bytes()) && bytes.Equal(e.Address, model.ETHBytes) {
		...
		// Record uncle reward
		if to := gethCommon.BytesToAddress(e.To); bytes.Equal(e.From, RewardToUncle.Bytes()) && bytes.Equal(e.Address, model.ETHBytes) {
