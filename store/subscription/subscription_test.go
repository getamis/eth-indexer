...
var _ = Describe("Database Test", func() {
	...
	Context("Subscription database", func() {
		...
		Context("BatchDelete", func() {
			...
			It("should delete subscriptions by group id", func() {
				addrs := [][]byte{
					common.HexToBytes("0xdfbba377a6d55d26d7dc6acd28279dc1f31308ed"),
					common.HexToBytes("0x52384b72f5582996d30f493ffc8518f6dc93f7c8"),
					common.HexToBytes("0x00f9252f87531fdb919a686e3e21d006a6304658"),
				}
				...
				By("Should be successful to delete two subscriptions", func() {
					err := store.BatchDelete(groupID, addrs[:2])
					Expect(err).Should(Succeed())
				})

				By("Should get only one subscription", func() {
					result, total, err := store.FindByGroup(groupID, &model.QueryParameters{
						Page:    1,
						Limit:   3,
						OrderBy: "created_at",
					...
					Expect(err).Should(Succeed())
					Expect(total).Should(Equal(uint64(1)))
					Expect(len(result)).Should(Equal(1))
					Expect(result[0].Group).Should(Equal(groupID))
					Expect(result[0].Address).Should(Equal(addrs[2]))
				})
			})

			It("should not delete subscriptions if group id and address are not matched", func() {
				addrs := [][]byte{
					common.HexToBytes("0x8c26348f63f9e008f0dd09a0ce1ed7caf6c1366b"),
				}
				subs := make([]*model.Subscription, len(addrs))
				for i, addr := range addrs {
					subs[i] = &model.Subscription{
				...
				By("Delete the subscriptions with not matching group id", func() {
					otherGroup := time.Now().UnixNano()
					err := store.BatchDelete(otherGroup, addrs)
					Expect(err).Should(Succeed())
				})

				By("Subscriptions should not be deleted", func() {
					result, total, err := store.FindByGroup(groupID, &model.QueryParameters{
						Page:    1,
						Limit:   1,
						OrderBy: "created_at",
					...
					Expect(len(result)).Should(Equal(1))
					Expect(result[0].Group).Should(Equal(groupID))
					Expect(result[0].Address).Should(Equal(addrs[0]))
				})
			})
		})
	})
